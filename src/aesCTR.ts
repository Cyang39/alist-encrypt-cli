// https://github.com/traceless/alist-encrypt/blob/main/node-proxy/src/utils/aesCTR.js
...
class AesCTR {
  ...
  constructor(password: string, sizeSalt: number) {
    ...
    if (password.length !== 32) {
      this.passwdOutward = crypto
        .pbkdf2Sync(this.password, "AES-CTR", 1000, 16, "sha256")
        .toString("hex");
    }
  ...
  async setPositionAsync(position: number): Promise<void> {
    ...
    this.iv = ivBuffer;
    const increment = parseInt(String(position / 16), 10);
